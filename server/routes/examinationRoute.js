...
router.post('/submit-exam', async (req, res) => {
  try {
    ...
    const existingAttempt = await ExamAttempted.findOne({
    ...
    const marksPerQuestion = exam.questions.length > 0 ? (totalMarks / exam.questions.length) : 0;
...
router.get('/examination', async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Failed to fetch pending exams' });
  }
});
